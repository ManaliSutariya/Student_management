...
exports.pending_fees =async(req,res)=>{
    if(req.query.name==undefined){
        var data= await studentdb.find().select('name coursename startingdate faculty_id installment').populate('coursename').populate('faculty_id')
    }
    else{
        var data= await studentdb.find({name:{$regex:'.*'+ req.query.name +'.*'}}).select('name coursename startingdate faculty_id installment').populate('coursename').populate('faculty_id')
    }
    var pending=[];
